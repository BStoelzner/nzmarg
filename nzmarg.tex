...
\defcitealias{hildebrandt18}{H20}
   ...
   \title{Weak lensing cosmology in the presence of redshift calibration uncertainty}
...
%The theoretical prediction of the observed signal is then given by
%\eqa{
%\xi_\pm^{(\alpha\beta)}(\theta)&= \sum_{i,j=1}^{N_z} A_i^\alpha A_j^\beta \left( x_\pm^{(ij)}(\theta) +  x_{\pm, {\rm GI}}^{(ij)}(\theta) +  x_{\pm, {\rm II}}^{(ij)}(\theta)   \right)\\
%&\equiv \sum_{i,j=1}^{N_z} A_i^\alpha A_j^\beta\; c_{ij}(\theta)\;,
%}
%where we again captured the two-point correlation function for Gaussian components at $z_i$ and $z_j$ in the quantity $c_{ij}(\theta)$. 
